...
### 8.10 2026-09-10 PDF 内容对齐补强

本轮根据《水脉千年-场景设计》PDF 第三章（39–59 页）和第四章（61–74 页）完成最小范围补强：

- 第三章仍保持五站七问，只修正几内亚水电题为“落差供电、生态流量、输电线路、鱼类繁殖期”四项，并为每一站增加一条短精灵引导、一条现场回响和一条转场提示。
- 第三章完成态明确发放水脉碎片③“同舟共济之纹”，不新增大面积完成卡片。
- 第四章开放题按“一道题、三个立场”计数为 `0 / 1` 或 `1 / 1`；三个选项仍然全部保留为可成立的思辨方向。
- 第四章航行路线由数据驱动，依次包含地球、月球、火星、木星、土星、冰卫星海洋和远方水迹。
- 终章完成态按顺序揭示四段精灵对白与“人法地、地法天、天法道、道法自然”四段哲理文字，并保存“天地人和 / 完整星河拼图”完成标记。
- 本轮不修改第一、第二章，不重新引入 WorkBuddy 生成的精灵动作或陌生人脸素材；背景图只在第三、第四章专属目录中按需替换。

### 8.11 2026-09-10 GSAP 视觉动效收敛

第三、第四章进入 `design-build` 的实现阶段。本轮使用官方 GSAP skills 的页面级时间线方法，将现有交互收敛为“场景切换时有序进入、状态变化时轻量反馈、内容始终可读”的动效语言：

- `WorldWater` 使用页面范围内的 GSAP timeline，让右侧面板、世界水路节点、当前节点和底部进度条按同一节奏进入；不改变站点位置，不新增大面积遮罩。
- `CosmicFuture` 使用页面范围内的 GSAP timeline，让背景层、轨道、信号点、核心、当前信号和进度条按“场景—关系—行动”顺序进入；碎片拼合只做短促的缩放/旋转反馈。
- 只动画 `transform`、`opacity` 和进度条缩放，不动画布局属性；`will-change` 仅放在实际动效目标上，并在 `prefers-reduced-motion: reduce` 下恢复自动状态。
- React 侧使用 `@gsap/react` 的 `useGSAP`、scope 和 `revertOnUpdate`，路由状态或题目切换后清理旧时间线，不遗留内联样式和事件。
- 这两章是地图纸面上的交互，不是滚动叙事，因此本轮不引入 ScrollTrigger；不自动播放视频，也不把陌生精灵动作重新接回页面。

桌面验收基准为 1280×720、1440×900 和 1920×1080：主标题、场景标注、右侧题板、主 CTA 与进度条不重叠，当前动作有明确视觉焦点，按钮命中区不小于 44px。移动端按当前范围暂缓专项布局，但不得破坏基础可读性和键盘焦点顺序。

### 8.12 2026-09-10 导览页选择性同步 main

本轮从最新 `origin/main`（`a12ec91`）只同步章节导览页所需的视觉层，不合并 main 的整套章节重构：

- 导览页保留当前 `src/data/chapters.ts` 的可用状态与路由，第三章继续进入 `/chapter-3`，第四章继续进入 `/chapter-4`。
- `ChapterOverview`、`StoryAtlas` 和 `StoryAtlasEffects` 采用地图纸面上的路径、节点、焦点和轻量进入动效；主图使用 `shanhai-water-chronicle.webp`，不再复制一套 C3/C4 的章节实现。
- 导览页交互验收覆盖：章节索引选择、澜澜导览对话、第三章 CTA、第四章 CTA、键盘焦点和桌面宽度下无横向溢出。
- 本轮明确不触碰 `WorldWater`、`CosmicFuture` 及其专属内容；后续若要更新 main，仍需按“导览层 / 共享组件 / 章节内容”分层挑选。

### 8.13 2026-09-10 第三、第四章语义化控件与字体层

本轮只针对第三、第四章补充一套可复用的章节行动控件和字体层：

- 新增 `ChapterActionButton`，用“勘、记、行、合、航、和、回”分别表达勘察、记录、继续、拼合、航行、完成和回看；按钮保留真实 `button` 语义、键盘焦点、忙碌状态和完成状态。
- `StatefulActionButton` 负责短暂的记录反馈，页面只传入语义 `intent`，不再为每个阶段复制一套按钮行为。
- 第三章采用“纸面标题字体 + 现场信息字体 + 数字字体”三层排版；第四章采用同一层级但让“航行/完成”使用更轻的水青强调色。字体均使用本地系统可用回退，不引入外部字体加载。
- 地图站点、宇宙信号和水脉碎片补充 `aria-pressed` / `data-status`；两章底部进度条改为带 `role="progressbar"` 与数值属性的可读进度控件。
- 选项按钮改成纸面横线与左侧行动线，选中状态同时改变位置、边线和文字，不依赖单一颜色；动效仍遵守 `prefers-reduced-motion`。

本轮只修改 `WorldWater`、`CosmicFuture` 和它们使用的章节控件，不影响首页、第一/第二章或章节导览页。
