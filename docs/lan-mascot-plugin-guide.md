...
```tsx
<LanMascotProvider showDefaultMascot={false}>
  {children}
  <LanFootingHost />
  <LanMascotHost />
</LanMascotProvider>
...
当前项目关闭了 main 中自带的全局默认精灵：只有页面调用 `useLanMascot()` 或 `useLanFooting()` 并完成注册后，才会显示对应精灵或脚底场景。这样可以让第三、第四章继续保留各自的既有实现，不会因为同步 main 自动多出一层全局覆盖。后续若需要全站导览，可将 `showDefaultMascot` 改为 `true`。
