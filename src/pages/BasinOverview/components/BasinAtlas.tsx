    ...
    <main className={`basin-atlas${selectedBasinId === null ? '' : ' basin-atlas--has-selection'}`}>
      ...
      <nav className="basin-atlas__markers" aria-label="流域入口">
        {basinAtlasMarkers.map((marker) => {
          ...
          const className = `basin-atlas__marker basin-atlas__marker--${marker.id}${selectedBasinId === marker.id ? ' is-selected' : ''}${marker.isAvailable ? '' : ' is-unavailable'}`;

          if (!marker.isAvailable) {
            return (
              <span key={marker.id} className={className} style={markerStyle} aria-label="珠江流域，水脉整理中，暂未开放">
                <span aria-hidden="true">{marker.name}</span>
                <span className="sr-only">水脉整理中，暂未开放</span>
              </span>
            );
          }
