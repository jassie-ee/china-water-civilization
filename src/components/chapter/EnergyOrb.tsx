...
import { ENERGY_ORB_FRAGMENT_SHADER, ENERGY_ORB_VERTEX_SHADER } from './energyOrbShaders';
...
/**
 * Local adaptation of ThreeUI's Energy Orb source.
 * Source reference: https://threeui.com/browse
 * Source revision supplied by the user: 03b1b8e2c440 / 3de7fdcb6239 / 7a88f26e4d8
 *
 * Only the self-contained WebGL orb and its 2D star layer are retained. The
 * page background, other Globe Collection variants, and ThreeUI runtime are
 * intentionally not included.
 */
export default function EnergyOrb({ className = '', ...props }: EnergyOrbProps) {
  ...
  useEffect(() => {
    ...
    const vertex = compile(gl, gl.VERTEX_SHADER, ENERGY_ORB_VERTEX_SHADER);
    const fragment = compile(gl, gl.FRAGMENT_SHADER, ENERGY_ORB_FRAGMENT_SHADER);
    ...
    const drawStars = (elapsed: number) => {
      ...
      const colorHue = fract((170 + options.hue) / 360) * 360;
      for (let index = 0; index < count; index += 1) {
        ...
        const twinkle = reducedMotion ? 0.72 : 0.56 + Math.sin(particleTime * (0.8 + star.depth) + star.phase) * 0.2;
        const alpha = Math.max(0.05, twinkle * (0.16 + star.depth * 0.34));
        const radius = Math.max(0.35, star.size * star.depth * Math.max(0.25, options.starSize));
        starContext.fillStyle = `hsla(${colorHue}, 35%, ${66 + star.depth * 20}%, ${alpha})`;
  ...
  const options = optionsRef.current;
  return (
    <div ref={hostRef} className={`chapter-energy-orb${className ? ` ${className}` : ''}`}>
      <canvas ref={starCanvasRef} className="chapter-energy-orb__stars" aria-hidden="true" />
      <canvas
        ref={canvasRef}
        className="chapter-energy-orb__shader"
        aria-hidden="true"
        style={{ opacity: options.opacity, filter: `brightness(${options.brightness})`, transform: `scale(${options.scale})` }}
      />
    </div>
  );
}
